...
const Kavitation = () => {
  ...
  return (
    <ServicePageLayout
      icon={Waves}
      title={language === 'de' ? "Ultraschallkavitation in München — sicheres Body-Contouring ohne Operation" : language === 'ru' ? "Кавитация в Мюнхене — безопасное моделирование тела с помощью ультразвука" : "Кавитация в Мюнхене — безопасное моделирование тела с помощью ультразвука"}
      subtitle={language === 'de' ? "Effektive Fettreduktion und Körperformung mit modernster Ultraschalltechnologie in München-Haidhausen" : language === 'ru' ? "Эффективная коррекция фигуры и моделирование тела с помощью современной ультразвуковой технологии в Мюнхене-Хайдхаузен" : "Эффективная коррекция фигуры и моделирование тела с помощью современной ультразвуковой технологии в Мюнхене-Хайдхаузен"}
      aboutTitle={language === 'de' ? "Was ist Ultraschallkavitation?" : language === 'ru' ? "Что такое ультразвуковая кавитация" : "Что такое ультразвуковая кавитация"}
      aboutDescription={[
        isGerman
      ...
      benefits={[
        language === 'de' ? 'Gezielte Fettreduktion ohne Operation' : language === 'ru' ? 'Целенаправленное уменьшение жира без операции' : 'Целенаправленное уменьшение жира без операции',
        language === 'de' ? 'Umfangsreduktion 2–6 cm pro Zone' : language === 'ru' ? 'Уменьшение объёмов 2–6 см за зону' : 'Уменьшение объёмов 2–6 см за зону',
        language === 'de' ? 'Reduzierung von Cellulite' : language === 'ru' ? 'Уменьшение целлюлита' : 'Уменьшение целлюлита',
        language === 'de' ? 'Völlig schmerzfrei und entspannend' : language === 'ru' ? 'Абсолютно безболезненно' : 'Абсолютно безболезненно',
        language === 'de' ? 'Keine Ausfallzeiten' : language === 'ru' ? 'Без реабилитации' : 'Без реабилитации',
        language === 'de' ? 'Sichere Alternative zur Liposuktion' : language === 'ru' ? 'Безопасная альтернатива липосакции' : 'Безопасная альтернатива липосакции',
        language === 'de' ? 'Sichtbare Ergebnisse nach 3–4 Behandlungen' : language === 'ru' ? 'Видимые результаты после 3–4 процедур' : 'Видимые результаты после 3–4 процедур'
      ]}
      steps={[
        {
          title: language === 'de' ? '1. Beratung und Vermessung' : language === 'ru' ? '1. Консультация и замеры' : '1. Консультація и замеры',
        ...
        {
          title: language === 'de' ? '3. Kavitation (30-60 Min.)' : language === 'ru' ? '3. Кавитация (30-60 мин.)' : '3. Кавитация (30-60 хв.)',
          description: isGerman
            ? 'Sanfte Behandlung mit Ultraschall. Völlig schmerzfrei.'
            : 'Мягкая обработка ультразвуком. Абсолютно безболезненно.'
        },
        {
          title: language === 'de' ? '4. Nachbehandlung' : language === 'ru' ? '4. Последующий уход' : '4. Последующий уход',
          description: isGerman
        ...
        {
          title: language === 'de' ? '5. Empfehlungen' : language === 'ru' ? '5. Рекомендации' : '5. Рекомендации',
          description: isGerman
      ...
      contraindications={[
        ...
        language === 'de' ? 'Herzschrittmacher oder implantierte elektronische Geräte' : language === 'ru' ? 'Кардиостимулятор или имплантированные электронные устройства' : 'Кардиостимулятор или имплантированные электронные устройства',
        language === 'de' ? 'Leber- oder Nierenerkrankungen (Fettstoffwechsel wird über Leber/Nieren abgebaut)' : language === 'ru' ? 'Заболевания печени или почек (метаболизм жиров происходит через печень/почки)' : 'Заболевания печени или почек (метаболизм жиров происходит через печень/почки)',
        language === 'de' ? 'Diabetes oder schwere Stoffwechselerkrankungen' : language === 'ru' ? 'Диабет или тяжёлые метаболические заболевания' : 'Диабет или тяжёлые метаболические заболевания',
        language === 'de' ? 'Thrombose, Krampfadern oder schwere Venenerkrankungen im Behandlungsbereich' : language === 'ru' ? 'Тромбоз, варикоз или тяжёлые заболевания вен в зоне обработки' : 'Тромбоз, варикоз или тяжёлые заболевания вен в зоне обработки',
        ...
        language === 'de' ? 'Frische Operationen im Behandlungsbereich (mind. 6 Monate Abstand)' : language === 'ru' ? 'Недавние операции в зоне обработки (мин. 6 месяцев после)' : 'Недавние операции в зоне обработки (хв. 6 месяцев после)',
        language === 'de' ? 'Metallimplantate, Spirale oder andere Fremdkörper im Behandlungsbereich' : language === 'ru' ? 'Металлические имплантаты, спираль или другие инородные тела в зоне обработки' : 'Металлические имплантаты, спираль или другие инородные тела в зоне обработки',
        language === 'de' ? 'Hautinfektionen, Entzündungen oder offene Wunden' : language === 'ru' ? 'Кожные инфекции, воспаления или открытые раны' : 'Кожные инфекции, воспаления или открытые раны',
        language === 'de' ? 'BMI über 30 (Kavitation ist keine Methode zur Gewichtsreduktion bei Adipositas)' : language === 'ru' ? 'ИМТ выше 30 (кавитация не является методом похудения при ожирении)' : 'ИМТ выше 30 (кавитация не является методом похудения при ожирении)'
      ]}
      faq={[
        {
          q: language === 'de' ? 'Wie viele Kavitationsbehandlungen brauche ich für sichtbare Ergebnisse?' : language === 'ru' ? 'Сколько процедур кавитации нужно для видимого результата?' : 'Сколько процедур кавитации нужно для видимого результата?',
          a: isGerman
        ...
        {
          q: language === 'de' ? 'Ist die Ultraschallkavitation schmerzhaft?' : language === 'ru' ? 'Ультразвуковая кавитация болезненна?' : 'Ультразвуковая кавитация болезненна?',
          a: isGerman
        ...
        {
          q: language === 'de' ? 'Wohin geht das Fett nach der Kavitation?' : language === 'ru' ? 'Куда девается жир после кавитации?' : 'Куда девается жир после кавитации?',
          a: isGerman
        ...
        {
          q: language === 'de' ? 'Nehme ich durch Kavitation ab (verliere ich Gewicht)?' : language === 'ru' ? 'Я похудею с помощью кавитации (потеряю вес)?' : 'Я похудею с помощью кавитации (потеряю вес)?',
          a: isGerman
        ...
        {
          q: language === 'de' ? 'Was ist der Unterschied zwischen Kavitation und Liposuktion (Fettabsaugung)?' : language === 'ru' ? 'В чём разница между кавитацией и липосакцией?' : 'В чём разница между кавитацией и липосакцией?',
          a: isGerman
        ...
        {
          q: language === 'de' ? 'Kann ich Kavitation mit anderen Behandlungen kombinieren?' : language === 'ru' ? 'Можно ли комбинировать кавитацию с другими процедурами?' : 'Можно ли комбинировать кавитацию с другими процедурами?',
          a: isGerman
        ...
        {
          q: language === 'de' ? 'Gibt es Ausfallzeiten nach der Kavitation?' : language === 'ru' ? 'Есть ли период реабилитации после кавитации?' : 'Есть ли период реабилитации после кавитации?',
          a: isGerman
        ...
        {
          q: language === 'de' ? 'Für wen ist Kavitation geeignet?' : language === 'ru' ? 'Для кого подходит кавитация?' : 'Для кого подходит кавитация?',
          a: isGerman
      ...
      ctaTitle={language === 'de' ? "Bereit für eine schlanke, definierte Silhouette in München?" : language === 'ru' ? "Готовы к стройному, очерченному силуэту в Мюнхене?" : "Готовы к стройному, очерченному силуэту в Мюнхене?"}
